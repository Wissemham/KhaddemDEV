...
public class EtudiantServiceImpl implements IEtudiantService{
    ...
    public void addAndAssignEtudiantToEquipeAndContract(Etudiant e, Integer idContrat, Integer idEquipe) {
        ...
        if(contrat!=null)log.info("contrat: "+contrat.getSpecialite());
        if(equipe!=null)log.info("equipe: "+equipe.getNomEquipe());
        log.info("etudiant: "+etudiant.getNomE()+" "+etudiant.getPrenomE()+" "+etudiant.getOp());
        List<Equipe> equipesMisesAjour = new ArrayList<>();
        if(contrat!=null)contrat.setEtudiant(etudiant);
    ...
    public List<Etudiant> getEtudiantsByDepartement(Integer idDepartement) {
        ...
        a.setIdDepartement(0);
        a.setEtudiants(new ArrayList<>()); // Return an empty list, not null
        Departement departement = departementRepository.findById(idDepartement).orElse(a);
